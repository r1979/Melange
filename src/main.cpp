...
const string strMessageMagic = "Spice Signed Message:\n";
...
static const int64 nReleaseBlocks = 10;
static const int64 nStartSubsidy = 18 * COIN;
static const int64 nMinSubsidy = .001 * COIN;
...
static const int64 nTargetTimespan = 8 * 120; // 16 minutes
static const int64 nTargetSpacing = 240; // 4 minutes
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SpiceMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SpiceMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running SpiceMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
